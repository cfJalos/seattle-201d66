...
function populateForm() {
  ...
  for (var i = 0; i < Product.allProducts.length; i++) {
      ...
      options.setAttribute('value', Product.allProducts[i].name);
...
function addSelectedItemToCart() {
  ...
  var selectElement = document.getElementById('items');
  var selectItem = selectElement.options[selectElement.selectedIndex].value;

  // TODO: get the quantity
  var selectQuantity = document.getElementById('quantity');
  var selectQuant = selectQuantity.value;
 

  // TODO: using those, add one item to the Cart
  cart.addItem(selectItem, selectQuant);
...
function updateCartPreview() {
  ...
  var prodElem = document.createElement('p');
  prodElem.textContent = item + ': ' + quantity;
  cartContent.appendChild(prodElem);
